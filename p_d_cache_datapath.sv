...
//valid array
cache_array #(.s_index(s_index), .width(1)) v_array [num_ways-1:0] (
    .i_clk({i_clk, i_clk, i_clk, i_clk}),
...
//dirty array
cache_array #(.s_index(s_index), .width(1)) d_array [num_ways-1:0] (
    .i_clk({i_clk, i_clk, i_clk, i_clk}),
    .rst_n({rst_n, rst_n, rst_n, rst_n}),
    .read({read_array_flag, read_array_flag, read_array_flag, read_array_flag}),
...
//tag array
cache_array #(.s_index(s_index), .width(s_tag)) tag_array [num_ways-1:0] (
    .i_clk({i_clk, i_clk, i_clk, i_clk}),
    .rst_n({rst_n, rst_n, rst_n, rst_n}),
    .read({read_array_flag, read_array_flag, read_array_flag, read_array_flag}),
...
//data array
cache_data_array #(.s_offset(s_offset), .s_index(s_index)) data_array [num_ways-1:0] (
    .i_clk({i_clk, i_clk, i_clk, i_clk}),
    .read({read_array_flag, read_array_flag, read_array_flag, read_array_flag}),
...
// LRU array width is now 3.
cache_array #(.s_index(s_index), .width(3)) LRU_array (
...
always_comb begin : HIT_MISS_DETERMINATION
    // Use === 1'b1 for each comparison to block X-propagation from uninitialized arrays.
    // A way can only HIT if both its tag matches AND its valid bit is provably 1.
    way_0_hit = (tag_array_0_dataout === prev_address[31:8]) && (v_array_0_dataout === 1'b1);
    way_1_hit = (tag_array_1_dataout === prev_address[31:8]) && (v_array_1_dataout === 1'b1);
    way_2_hit = (tag_array_2_dataout === prev_address[31:8]) && (v_array_2_dataout === 1'b1);
    way_3_hit = (tag_array_3_dataout === prev_address[31:8]) && (v_array_3_dataout === 1'b1);

    hit = way_0_hit | way_1_hit | way_2_hit | way_3_hit;

    // Priority encoder for data output
    casez ({way_3_hit, way_2_hit, way_1_hit, way_0_hit})
        4'b???1: dataout = data_array_0_dataout;
        4'b??10: dataout = data_array_1_dataout;
        4'b?100: dataout = data_array_2_dataout;
        4'b1000: dataout = data_array_3_dataout;
        default: dataout = '0;
    endcase
end
...
always_comb begin : DIRTY_DETERMINATION
    ...
    // Only dirty when ALL four ways are provably valid (=== 1'b1).
    // Using === blocks X-propagation from uninitialized _dataout registers.
    if ((v_array_0_dataout === 1'b1) && (v_array_1_dataout === 1'b1) &&
        (v_array_2_dataout === 1'b1) && (v_array_3_dataout === 1'b1)) begin
        case (LRU_array_dataout)
            3'b000, 3'b010: dirty_out = d_array_3_dataout;
            3'b001, 3'b011: dirty_out = d_array_2_dataout;
            3'b100, 3'b101: dirty_out = d_array_1_dataout;
            3'b110, 3'b111: dirty_out = d_array_0_dataout;
            default:        dirty_out = 1'b0;
        endcase
    end
end

endmodule : p_d_cache_datapath