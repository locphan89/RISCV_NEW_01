import rv32i_types::*;
// =============================================================================
// top_5stages_cache_full.sv  -- FIXED
//
// Fix list vs original:
//
// FIX-1 (Branch forwarding unit):
//   Changed from forward_branch_nopipe_unit_5stage to
//   forward_branch_nopipe_unit_6stage.  The 5-stage variant lacks wb_wr_reg /
//   wb_mem2reg_sig inputs, so loads followed immediately by a branch could not
//   forward the loaded value to the branch comparator.
//
// FIX-2 (id_rsb / id_rtb mux):
//   Added fwb_wb (bit[1]) case.  Without it, a value that just completed a
//   load (wb stage) was never forwarded to the branch comparator, making
//   bltu/bgeu/beq/bne after a load always read a stale regfile entry.
//
// FIX-3 (ALU forwarding source for MEM stage):
//   alu_in_target previously received raw mem_addr (the EX ALU address result).
//   For JAL/LUI at MEM stage the correct forwarded value is jal_result/imm,
//   not the address.  Replaced with mem_fw_data mux that selects the correct
//   result based on mem_cs control signals.
//
// FIX-4 (control_unit_std_full funct7 input):
//   Added .if_funct7_i(if_inst[30]) to distinguish add/sub and srl/sra.
//   Without this, sub and sra were decoded as add and srl respectively.
// =============================================================================
module top_5stages_cache_full
(
    input  logic                    i_clk,
    input  logic                    rst_n,
   ...
   // FIX-1: Use 6-stage variant which includes WB-stage (fwb_wb) forwarding.
   //        The 5-stage variant misses the WB path, so a load result in WB
   //        can't be forwarded to the branch comparator at ID.
   // =========================================================================
   forward_branch_nopipe_unit_6stage forward_branch_nopipe_unit_6stage_0 (
       .mem_wr_reg,
       .mem_is_rtype,
       .ex_imm2reg_sig (ex_cs.imm2reg_sig),
       .ex_pc2reg_sig  (ex_cs.pc2reg_sig),
       .ex_wr_reg,
       // FIX-1: wire WB stage for fwb_wb forwarding
       .wb_wr_reg,
   ...
   logic [PC_WIDTH-1:0]   id_jalr_addr;
   logic                  id_jalr_taken;

   pc_target_vr_full pc_target_vr_full_0 (
       .i_clk, .rst_n,
       .if_pc_plus     (if_pc_plus),
       .ex_branch_addr,
       .id_jmp_addr,
       .ex_branch_taken,
       .id_jmp_taken   (jmp_taken),
       .id_jalr_taken,
       .id_jalr_addr,
       .if_pc,
       .if_ready
   ...
   logic if_is_jalr_sig;
   logic id_is_jalr_sig;

   // FIX-4: pass if_funct7_i so sub/sra decode correctly
   control_unit_std_full control_unit_std_full_0 (
       .if_op_i    (if_op),
       .if_funct3_i(if_inst[14:12]),
       .if_funct7_i(if_inst[30]),       // FIX-4
       .if_ctrl_sig_o(if_cs),
       .is_jalr_sig(if_is_jalr_sig)
   );

   ifid_pipe_reg_vr_full ifid_pipe_reg_vr_full_0 (
       .flush_i         (flush),
       .i_clk, .rst_n,
       .if_inst_i       (if_inst),
       .if_pc_i         (if_pc),
   ...
   // ID STAGE
   // =========================================================================
   rv32i_word id_bimm, id_jimm, id_iimm;

   immediate_gen_full immediate_gen_full_0 (
       .inst   (id_inst),
       .imm    (id_imm),
       .b_imm  (id_bimm),
       .j_imm  (id_jimm),
   ...
   br_jmp_target_full br_jmp_target_full_0 (
       .id_pc_i        (id_pc),
       .id_rs1         (id_rsb),        // uses forwarded value for JALR
       .id_bimm_i      (id_bimm),
       .id_jimm_i      (id_jimm),
       .id_iimm_i      (id_iimm),
       .id_branch_addr_o(id_branch_addr),
       .id_jump_addr_o  (id_jmp_addr),
       .id_jalr_addr_o  (id_jalr_addr)
   );
   ...
   // -------------------------------------------------------------------------
   // Branch operand forwarding mux
   // FIX-2: Added fwb_wb (bit[1]) case so WB-stage load results can be
   //        forwarded to the branch comparator.
   // Priority: fwb_jal(4) > fwb_lui(3) > fwb_alu/MEM(2) > fwb_wb/WB(1) > regfile
   // -------------------------------------------------------------------------
   logic [DATA_WIDTH-1:0] ex_lui_result;
   logic [PC_WIDTH-1:0]   ex_jal_result;
   assign ex_lui_result = ex_imm;

   assign id_rsb = (fw_b1[4]) ? {{(DATA_WIDTH-PC_WIDTH){1'b0}}, ex_jal_result} : // fwb_jal
                   (fw_b1[3]) ? ex_lui_result :                                    // fwb_lui
                   (fw_b1[2]) ? mem_addr      :                                    // fwb_alu (MEM)
                   (fw_b1[1]) ? wb_result     :                                    // fwb_wb  (WB) [FIX-2]
                                id_rd1;                                            // regfile

   assign id_rtb = (fw_b2[4]) ? {{(DATA_WIDTH-PC_WIDTH){1'b0}}, ex_jal_result} : // fwb_jal
                   (fw_b2[3]) ? ex_lui_result :                                    // fwb_lui
                   (fw_b2[2]) ? mem_addr      :                                    // fwb_alu (MEM)
                   (fw_b2[1]) ? wb_result     :                                    // fwb_wb  (WB) [FIX-2]
                                id_rd2;                                            // regfile
   ...
   assign idex_wr_reg = (stall | stall_by_cache | id_mem_wr_sig | id_branch_sig) ?
                         '0 : id_wr_reg;
   ...
   assign flush = id_cs.jmp_sig | ex_branch_taken | id_is_jalr_sig;
   ...
   // FIX-3: compute the correct MEM-stage forwarding value.
   // For JAL: forward jal_result (PC+1), not the ALU address.
   // For LUI: forward the immediate, not the ALU address.
   // For normal ALU/load/store: forward the ALU result (= mem_addr).
   logic [DATA_WIDTH-1:0] mem_fw_data;
   assign mem_fw_data = mem_cs.pc2reg_sig  ? {{(DATA_WIDTH-PC_WIDTH){1'b0}}, mem_jal_result} :
                        mem_cs.imm2reg_sig ? mem_imm :
                                             mem_addr;

   alu_in_target alu_in_target_0 (
       .ex_rf_rd1      (ex_rd1),
       .ex_rf_rd2      (ex_rd2),
       .mem_addr       (mem_fw_data),   // FIX-3: use correct MEM forwarding value
       .wb_result,
       .ex_imm,
       .fw_alu_a, .fw_alu_b,
   ...
   rv32i_mem_wmask ex_byte_en;
   mask_gen mask_gen_0 (
       .alu_out         (ex_alu_result),
       .funct3          (ex_funct3),
       .write_read_mask (ex_byte_en)
   );
   ...
   exmem_pipe_reg_rv_cache_full exmem_pipe_reg_rv_cache_full_0 (
       .i_clk, .rst_n,
       .ex_cs       (exmem_cs),
       .ex_b        (ex_fw_b),
   ...
   rv32i_word               wb_imm;
   logic [FUNCT3_WIDTH-1:0] wb_funct3;

   memwb_pipe_reg_cache_full memwb_pipe_reg_cache_0 (
       .i_clk, .rst_n,
       .mem_cs       (memwb_cs),
       ...
       .mem_funct3, .wb_funct3
   );

   // =========================================================================
   // WB STAGE
   // lb/lbu/lh/lhu sign/zero extension
   // =========================================================================
   logic [31:0] load_data;
   logic [31:0] shifted;
   logic [1:0]  offset;

   assign offset  = wb_addr[1:0];
   assign shifted = wb_rdata >> (offset * 8);

   always_comb begin
      load_data = '0;
      unique case (wb_funct3)
         lb:  load_data = {{24{shifted[7]}},  shifted[7:0]};
         lbu: load_data = {24'b0,             shifted[7:0]};
         lh:  load_data = {{16{shifted[15]}}, shifted[15:0]};
         lhu: load_data = {16'b0,             shifted[15:0]};
         lw:  load_data = wb_rdata;
         default: ;
      endcase
   end

   assign wb_result = wb_cs.mem2reg_sig ? load_data     :
