...
// Cast state sang 2 bits ?? output
always_comb begin
    case(state)
        START:      cache_state_out = 2'b00;
        MISS:       cache_state_out = 2'b01;
        HIT:        cache_state_out = 2'b10;
        WRITE_BACK: cache_state_out = 2'b11;
        default:    cache_state_out = 2'b00;
    endcase
end
...
always_comb begin : state_actions
  ...
  case(state)
    START: begin
      // Handle cache hit directly in START state (avoids unnecessary MISS?HIT trip)
      if ((mem_read || mem_write) && (cache_pipeline_in.hit === 1'b1)) begin
        mem_resp       = 1'b1;
        LRU_array_load = 1'b1;
        if      (cache_pipeline_in.way_0_hit) LRU_array_datain = {1'b0, 1'b0, cache_pipeline_in.LRU_array_dataout[0]};
        else if (cache_pipeline_in.way_1_hit) LRU_array_datain = {1'b0, 1'b1, cache_pipeline_in.LRU_array_dataout[0]};
        else if (cache_pipeline_in.way_2_hit) LRU_array_datain = {1'b1, cache_pipeline_in.LRU_array_dataout[1], 1'b0};
        else if (cache_pipeline_in.way_3_hit) LRU_array_datain = {1'b1, cache_pipeline_in.LRU_array_dataout[1], 1'b1};
        if (cache_pipeline_in.mem_write) begin
          if      (cache_pipeline_in.way_0_hit) begin write_en_0_MUX_sel = cpu_write_cache; data_array_0_datain_MUX_sel = cpu_write_cache; d_array_0_load = 1'b1; d_array_0_datain = 1'b1; end
          else if (cache_pipeline_in.way_1_hit) begin write_en_1_MUX_sel = cpu_write_cache; data_array_1_datain_MUX_sel = cpu_write_cache; d_array_1_load = 1'b1; d_array_1_datain = 1'b1; end
          else if (cache_pipeline_in.way_2_hit) begin write_en_2_MUX_sel = cpu_write_cache; data_array_2_datain_MUX_sel = cpu_write_cache; d_array_2_load = 1'b1; d_array_2_datain = 1'b1; end
          else if (cache_pipeline_in.way_3_hit) begin write_en_3_MUX_sel = cpu_write_cache; data_array_3_datain_MUX_sel = cpu_write_cache; d_array_3_load = 1'b1; d_array_3_datain = 1'b1; end
        end
      end
    end
    ...
    HIT: begin
      // FIX: assert mem_resp unconditionally in HIT state.
      // We arrive here only from MISS after pmem_resp, so the cache fill
      // is complete. Do NOT gate on cache_pipeline_in.hit because the
      // pipeline register needs 1 extra cycle to propagate valid/tag,
      // so hit===1 is not visible yet on the first cycle of HIT state.
      address_mux_sel = curr_cpu_address;
      mem_resp        = 1'b1;
    end
...
always_comb begin : next_state_logic
  ...
  case(state)
    START: begin
      // Hit: stay in START (mem_resp already asserted in state_actions)
      // Miss: go to MISS
      if ((mem_read || mem_write) && (cache_pipeline_in.hit !== 1'b1))
        next_state = MISS;
    end
    ...
    HIT: begin
      // FIX: always return to START unconditionally.
      // The pipeline register needs 1 cycle to propagate the new valid/tag
      // after pmem fill. START will re-evaluate hit correctly next cycle.
      // Checking hit here causes livelock (hit=0 still) -> re-enters MISS forever.
      next_state = START;
    end
