...
(
  ...
  // logic                            rt_ifid_match;
   ...
   //------------------------------------------------------------------------
    ...
    forward_branch_nopipe_unit_5stage_0 (
   ...
   //------------------------------------------------------------------------
	...
	logic                            id_target_taken;
	
   /*pc_target_vr_full 
    pc_target_vr_full_0 (
       .i_clk(i_clk),
       .rst_n(rst_n),
       .id_pc_plus_i(if_pc_plus),
       .id_target_addr(id_target_addr),
    ...
    pc_target_vr_full_0 (
       ...
       .id_jmp_taken(jmp_taken),
		 .id_jalr_taken(id_jalr_taken),
		 .id_jalr_addr(id_jalr_addr),
       .if_pc(if_pc),
		 .if_ready(if_ready)
   );
   ...
   control_unit_std_full 
    control_unit_std_full_0 (
       .if_op_i(if_op),
   ...
   /*ifid_pipe_reg 
	...
	ifid_pipe_reg_vr_full 
    ifid_pipe_reg_vr_full_0 (
       .flush_i(flush),
       .i_clk(i_clk),
       .rst_n(rst_n),
       .if_inst_i(if_inst),
       ...
       .id_ctrl_sig_o(id_cs),
		 ...
		 .if_is_jalr_sig(if_is_jalr_sig),
		 .id_is_jalr_sig(id_is_jalr_sig)
   );
   ...
   /*br_jmp_target_full 
    br_jmp_target_full_0 (
       ...
       .ex_branch_taken(ex_branch_taken),
       .jmp_taken(jmp_taken),
		 .jalr_taken(id_is_jalr_sig),
		 
		 // Output
		 .target_addr_o(id_target_addr),
		 .target_taken_o(id_target_taken),
   ...
   assign idex_wr_reg = (stall | id_mem_wr_sig | id_branch_sig) ? 
                        '0 : id_wr_reg; 
   //------------------------------------------------------------------------
   ...
   //------------------------------------------------------------------------
	...
	exmem_pipe_reg_rv_cache_full 
    exmem_pipe_reg_rv_cache_full_0 (
       .i_clk(i_clk),
       .rst_n(rst_n),
       .ex_cs(exmem_cs),
       .ex_b(ex_fw_b),
   ...
   //------------------------------------------------------------------------
	...
	assign shifted  = wb_rdata >> ({offset, 3'b000});

	always_comb begin
		 load_data = '0;

		 unique case (load_funct3_t'(wb_funct3))
